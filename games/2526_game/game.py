...
Physics and sensing are ported directly from demo.html (the browser sandbox):
  - differential-drive kinematics (see physics.md)
  - SAT collision detection + positional push-out (see physics2.md)
  - 8-direction raycast distance sensors

The browser demo is a single-car sandbox. This module wraps that physics in a
two-player race: both cars run their own strategy every frame, and the first to
reach the finish zone within the time limit wins. If neither finishes in time,
nobody scores.

Coordinate system (matches demo.html):
...
class Game(BaseGame):
    ...
    def simulate(self, player_a_strategy, player_b_strategy):
        """Run the race until someone wins or time runs out.
